...
var isArray = function (json) {
    return typeof json === 'object' && json.constructor === Array;
};
...
module.exports = {
    ...
    eq: function (formatAssertErr) {
        return asyncWrap(function (actual, expected, stack) {
            ...
            if (eqRes === $maxDepthErr) {
                ...
                return Promise.reject(formatAssertErr(errText, errText, stack));
            }
            if (eqRes.pass) return Promise.resolve();

            return Promise.reject(formatAssertErr(actual, expected, stack));
        });
    }
};